// setup-v2.js — Run this once to write all updated files
// Usage: node setup-v2.js
const fs = require('fs');
const path = require('path');

// ── 1. Write generate-voiceover-v2.js ────────────────────────────────────────
const voiceScript = [
  "const fs = require('fs');",
  "const path = require('path');",
...
fs.writeFileSync('generate-voiceover-v2.js', voiceScript);
console.log('✓ Written: generate-voiceover-v2.js');

// ── 2. Update Root.tsx to 7200 frames ────────────────────────────────────────
const rootPath = path.join('src', 'Root.tsx');
let root = fs.readFileSync(rootPath, 'utf8');
// Replace any durationInFrames for LiveDemo with 7200
root = root.replace(
  /id="LiveDemo"[\s\S]*?durationInFrames=\{[^}]+\}/,
  (match) => match.replace(/durationInFrames=\{[^}]+\}/, 'durationInFrames={7200}')
);
fs.writeFileSync(rootPath, root);
console.log('✓ Updated: src/Root.tsx (7200 frames = 4 minutes)');

// ── 3. Verify the video file exists ──────────────────────────────────────────
const videoPath = path.join('public', 'game-demo.mp4');
if (fs.existsSync(videoPath)) {
  const size = (fs.statSync(videoPath).size / 1024 / 1024).toFixed(1);
  console.log('✓ Video found: public/game-demo.mp4 (' + size + ' MB)');
} else {
  console.log('⚠ Missing: public/game-demo.mp4');
  console.log('  Save short.mp4 from the chat and run:');
  console.log('  Copy-Item -Force "$env:USERPROFILE\\Downloads\\short.mp4" "public\\game-demo.mp4"');
}

console.log('\nNext steps:');
console.log('  1. node generate-voiceover-v2.js');
console.log('  2. npx remotion studio --force-new');
