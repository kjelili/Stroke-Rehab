/**
 * install-advert.js
 * Run from: C:\Users\DeLL\Desktop\Ready to deploy\Stroke Rehab\neuro-recover-videos
 * Command:  node install-advert.js
 *
 * This script:
 *   1. Writes AdvertDemo.tsx with real footage clips
 *   2. Checks that clip-*.mp4 files exist in public/
 *   3. Reports what's ready / missing
 */
const fs = require('fs');
const path = require('path');

// ─── 1. Write AdvertDemo.tsx ──────────────────────────────────────────────────
const advertTsx = `/**
...
const destPath = path.join('src', 'AdvertDemo', 'AdvertDemo.tsx');
fs.mkdirSync(path.dirname(destPath), { recursive: true });
fs.writeFileSync(destPath, advertTsx);
console.log('✅ Wrote', destPath);

// ─── 2. Check video clips ─────────────────────────────────────────────────────
const clips = ['clip-dashboard.mp4','clip-progress.mp4','clip-piano.mp4','clip-fingertap.mp4','clip-bubbles.mp4'];
const missing = clips.filter(c => !fs.existsSync(path.join('public', c)));

if (missing.length === 0) {
  console.log('✅ All video clips found in public/');
} else {
  console.log('\n⚠️  Missing video clips in public/:');
  missing.forEach(c => console.log('   -', c));
  console.log('\nThese clips need to be copied from Claude\'s outputs.');
  console.log('Download them from the chat and put them in: public\\');
}

// ─── 3. Check screenshots ─────────────────────────────────────────────────────
const shots = ['ss-clinician.jpg','ss-progress.jpg','ss-landing.jpg','ss-dashboard.jpg','ss-games.jpg'];
const missingShots = shots.filter(s => !fs.existsSync(path.join('public', s)));
if (missingShots.length === 0) {
  console.log('✅ All screenshots found in public/');
} else {
  console.log('⚠️  Missing screenshots:', missingShots.join(', '));
}

console.log('\n─────────────────────────────────────');
if (missing.length === 0) {
  console.log('🎬 Ready! Run: npx remotion studio --force-new');
  console.log('   Then select AdvertDemo in the left sidebar.');
} else {
  console.log('Fix missing files above, then run: npx remotion studio --force-new');
}
