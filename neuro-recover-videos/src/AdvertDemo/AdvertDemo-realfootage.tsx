 ...
 *
 * Structure (5400 frames = exactly 3:00 @ 30fps):
 *
 * ACT 1  0:00–0:28  (0–840)    THE PROBLEM — stark stats, 3 lines
 * ACT 2  0:28–1:05  (840–1950) THE APP — dashboard → piano live → finger tap → bubbles
 * ACT 3  1:05–1:45  (1950–3150) MEDGEMMA — AI layer, progress data, PDF report
 * ACT 4  1:45–2:20  (3150–4200) CLINICIAN — dashboard, FHIR, recovery curve
...
const C = {
  ...
  gold:    '#C8A44A',
  red:     '#E63F3F',
  blue:    '#4BA8FF',    // matches app native blue
  blueDim: 'rgba(75,168,255,0.15)',
  teal:    '#2DD4BF',
  tealDim: 'rgba(45,212,191,0.12)',
  pink:    '#E87AAE',    // MedGemma / Google Health
  pinkDim: 'rgba(232,122,174,0.13)',
};
...
// ── Helpers ───────────────────────────────────────────────────────────────────
const cl = (f: number, from: number, to: number, s: number, e: number) =>
  interpolate(f, [s, e], [from, to], { extrapolateLeft:'clamp', extrapolateRight:'clamp' });

const fi = (f: number, d = 0, dur = 25) => cl(f, 0, 1, d, d + dur);
const fo = (f: number, s: number, dur = 22) => cl(f, 1, 0, s, s + dur);
const ty = (f: number, d = 0, amt = 18, dur = 28) => cl(f, amt, 0, d, d + dur);
...
const Eyebrow = ({ text, color = C.teal, d = 0 }: { text:string; color?:string; d?:number }) => {
  ...
  return (
    <div style={{ fontSize:11,letterSpacing:4.5,textTransform:'uppercase',color,fontFamily:F.sans,fontWeight:500,opacity:fi(f,d,20) }}>
      {text}
    </div>
  );
};
...
// Video clip with overlay gradient and caption badge
const Clip = ({
  src, startFrom = 0, caption, badge, badgeColor = C.blue,
  overlay = 'linear-gradient(to top, rgba(0,0,0,0.80) 0%, rgba(0,0,0,0.10) 55%, rgba(0,0,0,0.45) 100%)',
  delay = 0, style = {},
}: {
  src: string; startFrom?: number; caption?: string; badge?: string; badgeColor?: string;
  overlay?: string; delay?: number; style?: React.CSSProperties;
}) => {
  const f = useCurrentFrame();
    ...
    <div style={{ position:'relative', borderRadius:10, overflow:'hidden', boxShadow:'0 20px 65px rgba(0,0,0,0.82)', transform:`scale(${sp})`, opacity:sp, ...style }}>
      ...
      {caption && (
        <div style={{ position:'absolute',bottom:14,left:16,right:16 }}>
          <div style={{ fontSize:13,color:C.white,fontFamily:F.serif,fontStyle:'italic',lineHeight:1.4 }}>{caption}</div>
        </div>
      )}
      {/* Game-name label top-right */}
    </div>
  );
};
...
const Shot = ({ src, delay = 0, rotate = 0, w = 500, glow = C.blueDim }: { src:string; delay?:number; rotate?:number; w?:number; glow?:string }) => {
  ...
  return (
    <div style={{
      width:w, borderRadius:10, overflow:'hidden',
      transform:`rotate(${rotate}deg) scale(${sp})`, opacity:sp,
      boxShadow:`0 22px 70px rgba(0,0,0,0.78), 0 0 0 1px rgba(255,255,255,0.07), 0 0 40px ${glow}`,
    }}>
  ...
  return (
    <AbsoluteFill style={{ background:C.black }}>
      ...
      <div style={{
        ...
        background:`radial-gradient(circle, rgba(230,63,63,0.18) 0%, transparent 65%)`,
        opacity: 0.55 + 0.45 * Math.sin(f / 32),
      }} />
      ...
      {f < 275 && (
        <AbsoluteFill style={{ display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',opacity:fo(f,250,22) }}>
          <div style={{ textAlign:'center' }}>
            <div style={{ fontSize:11,color:C.gold,letterSpacing:5,textTransform:'uppercase',fontFamily:F.sans,marginBottom:18,opacity:fi(f,12,20) }}>
              The problem with stroke rehab
            </div>
            <div style={{
              fontSize:172,fontWeight:700,color:C.red,lineHeight:0.88,
              fontFamily:F.serif,letterSpacing:-9,
              opacity:fi(f,18,16), textShadow:`0 0 100px rgba(230,63,63,0.4)`,
              transform:`scale(${cl(f,0.85,1,18,36)})`,
            }}>80%</div>
      ...
      {f >= 255 && (
        <AbsoluteFill style={{ display:'flex',alignItems:'center',justifyContent:'center',opacity:fi(f,255,28) }}>
          <div style={{ textAlign:'center',maxWidth:660 }}>
            ...
            <Rule d={380} w={52} />
            <div style={{ fontSize:18,color:C.dim,fontFamily:F.sans,fontWeight:300,opacity:fi(f,418,28),letterSpacing:0.3 }}>
              Introducing Neuro-Recover.
            </div>
          </div>
        </AbsoluteFill>
      )}
...
// ─────────────────────────────────────────────────────────────────────────────
// ACT 2 — THE APP IN ACTION (840–1950 = 37s)
// Real footage: dashboard → piano → finger tap → bubbles
// ─────────────────────────────────────────────────────────────────────────────
const Act2 = () => {
  const f = useCurrentFrame();
  return (
    <AbsoluteFill style={{ background:C.deep }}>
      ...
      {f < 250 && (
        <AbsoluteFill style={{ padding:'82px 80px',display:'flex',gap:52,alignItems:'center',opacity:fo(f,228,20) }}>
          <div style={{ flex:'0 0 auto',width:620 }}>
            <Clip src="clip-dashboard.mp4" startFrom={0} badge="LIVE SESSION" caption="Dashboard — AI coach + last session summary" delay={8} style={{ width:620 }} />
          </div>
          <div style={{ flex:1,display:'flex',flexDirection:'column',gap:18 }}>
            <Eyebrow text="Browser-native · No install required" color={C.blue} d={20} />
            <div style={{ fontSize:36,color:C.warm,fontFamily:F.serif,lineHeight:1.42,opacity:fi(f,30,28) }}>
              Open a tab. Allow camera access. Start your session.
            </div>
            <div style={{ fontSize:15,color:C.dim,fontFamily:F.sans,fontWeight:300,lineHeight:1.7,opacity:fi(f,65,26) }}>
              Works on any laptop or tablet. MediaPipe runs in the browser — no app, no hardware, no cloud dependency required.
            </div>
          </div>
        </AbsoluteFill>
      )}

      {/* Scene B: Piano live (235–570) */}
      {f >= 228 && f < 582 && (
        <AbsoluteFill style={{ opacity:Math.min(fi(f,228,26),fo(f,558,22)) }}>
          ...
          {/* Caption build — 3 phases */}
          <div style={{ position:'absolute',bottom:80,left:0,right:0,textAlign:'center',padding:'0 140px' }}>
            {f < 360 && (
              <div style={{ opacity:Math.min(fi(f,245,28),fo(f,340,22)) }}>
                <div style={{ fontSize:12,color:C.teal,letterSpacing:4,textTransform:'uppercase',marginBottom:10 }}>21 hand landmarks · 30 fps</div>
            ...
            {f >= 345 && f < 482 && (
              <div style={{ opacity:Math.min(fi(f,345,26),fo(f,460,20)) }}>
                <div style={{ fontSize:32,color:C.white,fontFamily:F.serif,fontStyle:'italic',lineHeight:1.4 }}>"Hand detected — extend fingers or tap keys"</div>
                <div style={{ fontSize:14,color:C.dim,fontFamily:F.sans,marginTop:10 }}>The system sees exactly what the clinician cannot.</div>
              </div>
            )}
            {f >= 465 && (
              <div style={{ opacity:Math.min(fi(f,465,26),fo(f,548,20)) }}>
      ...
      {f >= 760 && (
        <AbsoluteFill style={{ opacity:fi(f,760,26) }}>
          ...
          <div style={{ position:'absolute',top:68,right:52,opacity:fi(f,775,18) }}>
            <div style={{ background:'rgba(75,168,255,0.15)',border:'1px solid rgba(75,168,255,0.28)',borderRadius:5,padding:'6px 14px' }}>
              <span style={{ fontSize:12,color:C.blue,letterSpacing:2,fontFamily:F.sans }}>Adaptive difficulty · real-time</span>
            </div>
          </div>
          <div style={{ position:'absolute',bottom:80,left:0,right:0,textAlign:'center',padding:'0 160px' }}>
            {f < 900 && (
              <div style={{ opacity:Math.min(fi(f,772,26),fo(f,882,20)) }}>
                <div style={{ fontSize:30,color:C.white,fontFamily:F.serif,fontStyle:'italic',lineHeight:1.45 }}>Pinch or tap to pop. Difficulty adapts: slower and larger if fatigued, smaller and faster if improving.</div>
              </div>
            )}
            {f >= 880 && (
              <div style={{ opacity:fi(f,880,26) }}>
...
// ─────────────────────────────────────────────────────────────────────────────
// ACT 3 — MEDGEMMA ENGINE (1950–3150 = 40s)
// The AI layer — typewriter analysis, progress data, PDF export footage
// ─────────────────────────────────────────────────────────────────────────────
const Act3 = () => {
  const f = useCurrentFrame();
  return (
    <AbsoluteFill style={{ background:C.ink }}>
      ...
      <div style={{
        ...
        width:820,height:520,borderRadius:'50%',
        background:`radial-gradient(ellipse, rgba(232,122,174,0.09) 0%, transparent 68%)`,
        opacity: 0.6 + 0.4*Math.sin(f/48),
      }} />

      {/* MedGemma reveal (0–290) */}
      {f < 300 && (
        <AbsoluteFill style={{ display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',opacity:fo(f,278,22) }}>
          <div style={{ textAlign:'center' }}>
            <div style={{ fontSize:11,color:C.pink,letterSpacing:6,textTransform:'uppercase',fontFamily:F.sans,marginBottom:22,opacity:fi(f,15,22) }}>
              Powered by Google Health AI
            </div>
            <div style={{ fontSize:82,fontFamily:F.serif,color:C.warm,fontWeight:400,letterSpacing:1,opacity:fi(f,28,35),textShadow:`0 0 60px rgba(232,122,174,0.25)` }}>
              MedGemma
            </div>
            <div style={{ fontSize:15,color:C.dim,fontFamily:F.sans,fontWeight:300,letterSpacing:3.5,marginTop:14,opacity:fi(f,68,28) }}>
              HAI-DEF · Health AI Developer Foundations · Google Cloud Vertex AI
            </div>
            ...
            <div style={{ fontSize:20,color:C.bright,fontFamily:F.serif,fontStyle:'italic',opacity:fi(f,145,30),maxWidth:600,margin:'0 auto',lineHeight:1.6 }}>
              Clinical reasoning. Agentic session orchestration. SOAP notes. ICD-10 tagging. FHIR export. Runs on Vertex AI — or fully offline on any hospital machine.
            </div>
          </div>
        </AbsoluteFill>
      )}

      {/* Live agentic output + progress footage (280–720) */}
      {f >= 272 && f < 732 && (
        <AbsoluteFill style={{ padding:'88px 90px',display:'flex',gap:52,alignItems:'center',opacity:Math.min(fi(f,272,26),fo(f,710,22)) }}>
          {/* Left: typewriter AI output */}
          <div style={{ flex:1 }}>
            <Eyebrow text="Agentic clinical loop — real tool calls" color={C.pink} d={282} />
            <div style={{ height:18 }} />
            <div style={{
              background:'rgba(232,122,174,0.05)',border:'1px solid rgba(232,122,174,0.18)',
              borderRadius:9,padding:'22px 26px',opacity:fi(f,290,20),
            }}>
          ...
          {/* Right: progress footage */}
          <div style={{ flex:1 }}>
            <Clip src="clip-progress.mp4" startFrom={0} badge="LIVE" caption="Progress page — 8 sessions, score over time + PDF export" delay={305} style={{ width:'100%' }} />
          </div>
        </AbsoluteFill>
      )}

      {/* HAI-DEF model trio (710–1200) */}
      {f >= 702 && (
        <AbsoluteFill style={{ padding:'88px 100px',display:'flex',flexDirection:'column',justifyContent:'center',gap:30,opacity:fi(f,702,26) }}>
          <Eyebrow text="Three HAI-DEF models — one integrated platform" color={C.pink} d={715} />
          <div style={{ display:'flex',gap:24 }}>
            {[
              { name:'MedGemma',  role:'Reports · SOAP notes · Agentic tools · ICD-10 · FHIR · Coaching',       status:'Live on Vertex AI',         color:C.pink },
              ...
              <div key={i} style={{
                flex:1,background:'rgba(255,255,255,0.030)',border:'1px solid rgba(255,255,255,0.075)',
                borderRadius:10,padding:'22px 20px',
                opacity:fi(f,720+i*55,26),
                transform:`translateY(${ty(f,720+i*55,18,26)})`,
              }}>
          ...
          <div style={{
            background:'rgba(232,122,174,0.055)',border:'1px solid rgba(232,122,174,0.16)',
            borderRadius:8,padding:'14px 22px',opacity:fi(f,900,26),
          }}>
            <span style={{ fontSize:13,color:C.pink,fontFamily:F.sans }}>
              ✦ Offline mode: Docker + Ollama + MedGemma 2B/8B. No data leaves the device. Designed for NHS air-gapped environments. GDPR compliant by architecture.
            </span>
          </div>
        </AbsoluteFill>
      )}
...
// ─────────────────────────────────────────────────────────────────────────────
// ACT 4 — CLINICIAN VIEW (3150–4200 = 35s)
// Real clinician dashboard footage + screenshots
// ─────────────────────────────────────────────────────────────────────────────
const Act4 = () => {
  const f = useCurrentFrame();
  return (
    <AbsoluteFill style={{ background:'#0A0A14' }}>
      ...
      {/* Clinician + recovery curve (0–480) */}
      {f < 492 && (
        <AbsoluteFill style={{ padding:'85px 80px',display:'flex',gap:50,alignItems:'center',opacity:fo(f,468,22) }}>
          <div style={{ flex:1.25 }}>
            <Shot src="ss-clinician.jpg" delay={15} w={560} glow={C.tealDim} />
          </div>
          <div style={{ flex:1,display:'flex',flexDirection:'column',gap:20 }}>
            <Eyebrow text="Clinician dashboard" color={C.teal} d={22} />
            <div style={{ fontSize:36,color:C.warm,fontFamily:F.serif,lineHeight:1.42,opacity:fi(f,32,30) }}>
              See the recovery curve. Catch the regression. Before the next appointment.
            </div>
            <div style={{ fontSize:15,color:C.dim,fontFamily:F.sans,fontWeight:300,lineHeight:1.7,opacity:fi(f,68,26) }}>
              Feb 15: grab-cup score 8 — flagged automatically. MedGemma adjusted intensity. Feb 16 onwards: recovery resumes. The clinician knew on day three.
            </div>
            {[
      ...
      {/* Progress screenshot + metrics (470–1050) */}
      {f >= 462 && (
        <AbsoluteFill style={{ padding:'85px 80px',display:'flex',gap:50,alignItems:'center',opacity:fi(f,462,26) }}>
          <div style={{ flex:1,display:'flex',flexDirection:'column',gap:18 }}>
            ...
            <Shot src="ss-progress.jpg" delay={482} w={500} glow={C.tealDim} />
          </div>
          <div style={{ flex:1,display:'flex',flexDirection:'column',gap:24 }}>
            <Eyebrow text="Objective clinical outcomes" color={C.blue} d={520} />
            <div style={{ fontSize:32,color:C.warm,fontFamily:F.serif,lineHeight:1.42,opacity:fi(f,530,28) }}>
              Not self-reported. Not estimated. Measured every session, automatically.
            </div>
            {/* Metric grid */}
            <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:16,opacity:fi(f,575,26) }}>
              {[
                { val:'+30%', label:'Finger ROM',      up:true  },
                { val:'-18%', label:'Tremor index',    up:true  },
                { val:'8/8',  label:'Sessions kept',   up:true  },
                { val:'12min',label:'Fatigue threshold (up from 7)', up:true },
              ].map((m,i) => (
...
// ─────────────────────────────────────────────────────────────────────────────
// ACT 5 — IMPACT + CTA (4200–5400 = 40s)
// ─────────────────────────────────────────────────────────────────────────────
const Act5 = () => {
  const f = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{ background:C.black }}>
      ...
      <AbsoluteFill style={{ display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center' }}>
        ...
        <div style={{ textAlign:'center' }}>
          {[
            { t:'Every rep, measured.',       c:C.warm },
            { t:'Every session, remembered.', c:C.warm },
            { t:'Every patient, seen.',        c:C.teal },
          ].map((line,i) => (
            <div key={i} style={{
              fontSize:54,fontFamily:F.serif,color:line.c,
              fontWeight:400,lineHeight:1.4,letterSpacing:0.5,
              opacity:fi(f, 18+i*65, 40),
              transform:`translateY(${ty(f, 18+i*65, 18, 40)})`,
            }}>{line.t}</div>
          ))}
        </div>
        ...
        {/* Badge row */}
        <div style={{ display:'flex',gap:18,marginBottom:30,opacity:fi(f,275,35) }}>
          {['MedGemma · HAI-DEF','FHIR R4 · NICE NG236','Offline-capable · GDPR','Open source · MIT'].map((b,i) => (
            <div key={i} style={{ fontSize:11,color:C.dim,letterSpacing:2,textTransform:'uppercase',fontFamily:F.sans,opacity:fi(f,282+i*25,22) }}>
              {b}{i<3 && <span style={{ margin:'0 9px',opacity:0.3 }}>·</span>}
            </div>
          ))}
        </div>
        ...
        <div style={{ textAlign:'center',opacity:fi(f,328,38),marginBottom:36 }}>
          ...
          <div style={{ fontSize:11,color:'rgba(255,255,255,0.20)',letterSpacing:3,textTransform:'uppercase',marginTop:8,fontFamily:F.sans,opacity:fi(f,358,28) }}>
            MedGemma Impact Challenge · Google Health AI · 2026
          </div>
        </div>

        {/* GitHub CTA */}
        <div style={{ opacity:fi(f,388,45),textAlign:'center' }}>
          <div style={{ fontSize:11,color:C.dim,letterSpacing:3,textTransform:'uppercase',marginBottom:16,fontFamily:F.sans }}>
            Open source · Free · Ready to deploy
          </div>
          <div style={{
            display:'inline-flex',alignItems:'center',gap:16,
            border:'1px solid rgba(255,255,255,0.22)',borderRadius:6,
            padding:'17px 36px',background:'rgba(255,255,255,0.038)',
            boxShadow:'0 0 48px rgba(45,212,191,0.07)',
          }}>
            ...
            <span style={{ fontSize:18,color:C.warm,fontFamily:F.sans,fontWeight:400,letterSpacing:0.8 }}>
...
export const AdvertDemo: React.FC = () => (
  <AbsoluteFill>
    {/* Audio — run measure-and-fix.js on ad-*.mp3 to fix exact timings */}
